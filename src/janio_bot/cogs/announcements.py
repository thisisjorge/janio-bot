...
class AnnouncementsCog(
    ...
    async def configure(
        ...
        if len(content) > 2000:
            await interaction.response.send_message(
                embed=make_error_embed("A mensagem pode ter no máximo 2000 caracteres."), ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            embed=make_success_embed(
                f"Aviso ativado em {canal.mention} a cada **{setting.interval_seconds} segundos**.\n"
    ...
    async def enable(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_message(embed=make_success_embed("Aviso ativado."), ephemeral=True)
    ...
    async def disable(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_message(embed=make_success_embed("Aviso desativado."), ephemeral=True)
    ...
    async def status(self, interaction: discord.Interaction) -> None:
        ...
        if setting is None:
            await interaction.response.send_message(
                embed=make_error_embed("O aviso ainda não foi configurado."), ephemeral=True
            )
            return
        embed = make_embed(
            title="⏰ Aviso recorrente",
            description="",
            color=Colors.SUCCESS if setting.enabled else discord.Color.light_grey(),
        )
        embed.add_field(
    ...
    async def test(self, interaction: discord.Interaction) -> None:
        ...
        if setting is None:
            await interaction.response.send_message(
                embed=make_error_embed("Configure o aviso primeiro."), ephemeral=True
            )
            return
        ...
        if not isinstance(channel, (discord.TextChannel, discord.Thread)):
            await interaction.response.send_message(
                embed=make_error_embed("O canal configurado não aceita mensagens."), ephemeral=True
            )
            return
        await channel.send(
            setting.message,
        ...
        await interaction.response.send_message(embed=make_success_embed("Aviso de teste enviado."), ephemeral=True)
